def longest_sentence(string)
  longest = string.split(/[.!?]/).max { |a, b| a.length <=> b.length }.strip
  "#{longest} (#{longest.split(' ').length} words)"
end

def longest_word(string)
  string.split(/\b/).max_by { |word| word.gsub(/\s+/, '').length }
end

puts longest_word(%(Project Gutenberg's Frankenstein, by Mary Wollstonecraft (Godwin) Shelley
